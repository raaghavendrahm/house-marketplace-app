// Everything from CreateListing is copied and necessary modifications are done
...
const EditListing = () => {
  ...
  // Redirect if listing is not that user's:
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      if (docSnap.exists()) {
        ...
        setFormData({ ...docSnap.data(), address: docSnap.data().location }); // to keep the listing data (instead of default form data) on which edit is clicked. And, as address is handled separately with location, it is added separately (else address will be blank).
        setLoading(false);
      } else {
        navigate('/');
  ...
  const onSubmit = async (e) => {
    ...
    // Update Listing:
    // Unlike earlier (as done in create listing as shown above), instead of adding a document, updating it:
    const docRef = doc(db, 'listings', params.listingId);
